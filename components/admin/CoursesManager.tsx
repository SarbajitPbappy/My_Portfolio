...
export default function CoursesManager() {
  ...
  const handleDeleteAll = async () => {
    ...
    const confirmMessage = `Are you sure you want to delete ALL ${items.length} courses? This action cannot be undone!`
    if (!confirm(confirmMessage)) return

    try {
      const deletePromises = items.map(item => 
        item.id ? fetch(`/api/courses/${item.id}`, { method: 'DELETE' }) : Promise.resolve()
      )
      ...
      alert(`Successfully deleted all ${items.length} courses!`)
    } catch (error) {
